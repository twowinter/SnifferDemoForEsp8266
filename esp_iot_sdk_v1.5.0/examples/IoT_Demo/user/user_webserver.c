...
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR
ws_send(void *arg, bool responseOK, char *psend)
{
    uint16 length = 0;
    char *pbuf = NULL;
    char httphead[256];
...
LOCAL void ICACHE_FLASH_ATTR
webpage_send(void *arg)
{
    char *pbuf = NULL;
    pbuf = (char *)os_zalloc(1024);
    struct espconn *ptrespconn = arg;

    os_sprintf(pbuf, "<center><p> LED Config");
    os_sprintf(pbuf, "<form>");
    os_sprintf(pbuf, "para1£º<input type=\"text\" name=\"para1\"><br />");
    os_sprintf(pbuf, "</form>");
    ws_send(ptrespconn, true, pbuf);
    os_free(pbuf);
    pbuf = NULL;
}
...
//        os_printf(precvbuffer);
        ...
        switch (pURL_Frame->Type) {
            case GET:
                os_printf("We have a GET request.%s\n", pURL_Frame->pSelect);
    			...
    			} else if (os_strlen(pURL_Frame->pSelect) == 0) {
              os_printf("View root.\n");
              webpage_send(ptrespconn);
    			} else {
                    response_send(ptrespconn, false);
                }
